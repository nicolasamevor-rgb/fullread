<form id="formLivre" action="{{ route('catalogue.create') }}" class="mt-6 {{ $visible }}" method="POST">
    @csrf

    <div class="mb-4">
        <label for="titre" class="block text-sm text-gray-700 font-meduim">Titre</label>
        <input type="string" name="titre" id="titre"
    ...
    <div class="mb-4">
        ...
        <input type="string" name="auteur" id="auteur"
            class=" block w-full px-3 py-2 border rounded-lg focus:outline-none focus:ring-2 focus:ring-gray-600 shadow-md "
            required>
        @error('auteur')
            <span class="text-red-500 text-sm">{{ $message }}</span>
...
<script>
    document.getElementById('addbok').addEventListener('click', function () {
        document.getElementById('formLivre').classList.remove('hidden')
        document.getElementById('formLivre').classList.add('block')
    });
</script>